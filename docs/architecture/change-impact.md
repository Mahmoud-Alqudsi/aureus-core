...
2. **Runtime Installation Dependency (`Package::hasDependencies([...])`)**: Declared in `*ServiceProvider::configureCustomPackage()`. Consumed exclusively by `InstallCommand` to enforce database migration order and prerequisite checks.
...
- `Webkul\Account\Models\PartnerCompanyProperty`: A concrete Eloquent model defined in the `accounts` plugin (`plugins/webkul/accounts/src/Models/PartnerCompanyProperty.php`) backed by the physical database table `partners_partner_company_properties`, managing partner accounting properties per company.
...
│ RestrictToAllowedCompanies   │ Model trait applying AllowedCompanyScope to restrict company queries.   │
...
Record-level access control resolves ownership dynamically via `Webkul\Security\Traits\HasOwnershipScope` and `OwnerSource`:
...
| **Model - Ownership Trait** | Modifying `HasOwnershipScope` trait or `OwnerSource` configuration | Record-level authorization, `OwnershipScope`, Bouncer filtering | **CRITICAL** | Unauthorized user access; records visible to wrong team members; Bouncer query failure. | Security Lead | Verify `OwnerSource` resolution kind (`column`, `relation`, `pivot`, `followers`). | Pest authorization test exercising `INDIVIDUAL`, `GROUP`, and `GLOBAL` roles. | `docs/security/ownership-scopes.md`, `docs/security/authorization.md`. |
...
| **Plugin - Runtime Dep** | Modifying `$package->hasDependencies([...])` array | Plugin installation ordering, prerequisite validation in CLI | **HIGH** | Plugin fails to install due to missing dependencies; migrations run out of order. | Core Architect | Trace `InstallCommand` dependency resolution DAG. | Test installing plugin on clean system via `php artisan plugin:install`. | `docs/architecture/plugin-registry.md`, `docs/ai/plugin-rules.md`. |
