...
| **TERM-003** | Livewire framework version installed is v4.3.3; `AGENTS.md` v3 statement is stale | VERIFIED | HIGH | `composer.lock` specifies `livewire/livewire` version `v4.3.3` | composer.lock | livewire/livewire | root | framework | 2026-09-04 | Phase 11 Auditor | Source of truth hierarchy dictates composer.lock overrides AGENTS.md |
...
| **TERM-005** | Dual User Model: `App\Models\User` extends `Webkul\Security\Models\User` | VERIFIED | HIGH | Class declaration: `class User extends \Webkul\Security\Models\User` | app/Models/User.php | User | security | architecture | 2026-09-04 | Phase 11 Auditor | Security User holds domain traits; App User is authenticatable |
...
| **SEC-003** | HTTP request tenant validation enforced via `RestrictToAllowedCompanies` middleware | VERIFIED | HIGH | Middleware inspects active company and aborts HTTP 403 if unauthorized | plugins/webkul/security/src/Http/Middleware/RestrictToAllowedCompanies.php | RestrictToAllowedCompanies | security | security | 2026-09-04 | Phase 11 Auditor | Applied to admin panel routes to prevent URL company ID tampering |
...
| **DEP-002** | Plugin installation order governed exclusively by `$package->hasDependencies([...])` | VERIFIED | HIGH | `InstallPluginCommand` builds installation DAG from `hasDependencies()` | plugins/webkul/plugin-manager/src/Console/Commands/InstallPluginCommand.php | InstallPluginCommand | plugin-manager | lifecycle | 2026-09-04 | Phase 11 Auditor | Composer require has zero impact on plugin installation or migration order |
...
| **CORR-001** | `AGENTS.md:19` states Livewire v3; `composer.lock` proves Livewire v4.3.3 is installed | PROPOSED | HIGH | Discrepancy between root guideline documentation and actual installed package | AGENTS.md | livewire/livewire | root | documentation | 2026-09-04 | Phase 11 Auditor | Discrepancy documented in Phase 10; pending update in root documentation |
...
2. **Runtime Plugin Tier (`Package::hasDependencies([...])`)**: Declared in `*ServiceProvider::configureCustomPackage()`. Consumed exclusively by `InstallPluginCommand` to determine installation sequence and prerequisite availability. None of the 9 core plugins declare runtime dependencies.
