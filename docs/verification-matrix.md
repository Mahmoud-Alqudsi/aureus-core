...
| Upstream synchronization of `upstream/master` target `4033c7055` into `develop` | VERIFIED | `a0c11c67b` merges `2942d1cb8` (origin/develop) and `4033c7055` (upstream/master); zero merge conflicts; `composer validate --strict` passed; `git diff --check` passed; Filament v5.8.1 assets updated | 2026-09-23 | Pull Request CI run on GitHub Actions (`pest_tests`, `translations_check`, `playwright_tests`). |

The 2026-09-18 synchronization range contained eight migrations and no Composer manifest changes. The 2026-09-23 synchronization range contains zero migrations, no modifications to `.github/workflows/`, and an upgrade of Filament from v5.7.6 to v5.8.1 in `composer.lock`. Downstream branding and privacy behaviors in `InstallCommand` remain preserved.
